...
class SetupStatusView(APIView):
    ...
    def get(self, request):
        ...
        return Response({
            ...
            "steps": steps,
            "readiness": readiness,
            "allowed_actions": SetupAllowedActions.for_user(request.user),
        })

class SetupHealthView(APIView):
    permission_classes = [IsAuthenticated, IsCompanySetupAdmin]
...
class SetupStepCompleteView(APIView):
    ...
    def post(self, request):
        ...
        state = SetupStateService.complete_step(
            company=request.user.company, step=step, actor=request.user, request=request
        )
        return Response({"current_step": state.current_step, "completed_steps": state.completed_steps})

class SetupStepSkipView(APIView):
    permission_classes = [IsAuthenticated, IsCompanySetupAdmin]

    def post(self, request):
        state = SetupStateService.skip_step(
        ...
        return Response({"skipped_steps": state.skipped_steps})

class SetupFinishView(APIView):
    permission_classes = [IsAuthenticated, IsCompanySetupAdmin]

    def post(self, request):
        state = SetupStateService.finish(
            company=request.user.company, actor=request.user, request=request
        )
        return Response({"onboarding_completed": state.onboarding_completed})
