    ...
    def validate(self, attrs):
        ...
        for field in ("workflow", "form_template"):
            ...
            if value and value.company_id != company.id:
                raise serializers.ValidationError({field: "This item belongs to another company."})
        return attrs
...
class RequestTypeDefinitionSerializer(CompanyOwnedConfigurationSerializer):
    ...
    def validate(self, attrs):
        ...
        for enabled, required in (("amount_enabled", "amount_required"), ("quantity_enabled", "quantity_required"), ("needed_by_enabled", "needed_by_required"), ("attachments_allowed", "attachments_required")):
            if attrs.get(required, getattr(self.instance, required, False)) and not attrs.get(enabled, getattr(self.instance, enabled, False)):
                raise serializers.ValidationError({required: "A required field must be enabled."})
        return attrs
...
class ApprovalRouteSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ApprovalRoute
        fields = ("id", "company", "name", "description", "workflow", "is_active", "steps")
...
class ReportingProcessSerializer(serializers.ModelSerializer):
    ...
    approval_route_name = serializers.CharField(source="approval_route.name", read_only=True)

    class Meta:
        model = ReportingProcess
        fields = ("id", "company", "name", "description", "schedule", "template", "approval_route", "approval_route_name", "is_active")
